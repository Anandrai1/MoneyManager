...
public class Expense extends RealmObject {
    ...
    public static float getTotalExpensesByDateMode(@IDateMode int dateMode) {
        ...
        return totalIncome.sum("total").floatValue() - totalExpense.sum("total").floatValue();
    }

    public static float getExpensesByDateMode(@IDateMode int dateMode) {
        Date dateFrom;
        Date dateTo;
        switch (dateMode) {
            case IDateMode.MODE_TODAY:
    ...
    public static float getIncomeByDateMode(@IDateMode int dateMode) {
        Date dateFrom;
        Date dateTo;
        switch (dateMode) {
            case IDateMode.MODE_TODAY:
    ...
    public static float getAvailableAmount() {
        RealmResults<Expense> totalExpense = getAllExpensesList(IExpensesType.MODE_EXPENSES);
        RealmResults<Expense> totalIncome = getAllExpensesList(IExpensesType.MODE_INCOME);
        return totalIncome.sum("total").floatValue() - totalExpense.sum("total").floatValue();
    }
    ...
    public static RealmResults<Expense> getAllExpensesList(@IExpensesType Integer type) {
        RealmQuery<Expense> realmQuery = RealmManager.getInstance().getRealmInstance()
                .where(Expense.class);
        if (type != null) realmQuery.equalTo("type", type);
